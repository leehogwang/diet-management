...
class ImageProcessor {
  ...
  /// bytes에서 이미지를 로드 (웹 호환)
  static img.Image? loadImageFromBytes(Uint8List bytes) {
    try {
      return img.decodeImage(bytes);
    } catch (e) {
      debugPrint('이미지 로드 실패: $e');
  ...
  /// 음식 사진 자동 보정 (빠른 처리)
  static img.Image autoEnhanceForFood(img.Image original) {
    debugPrint('자동 보정 시작: ${original.width}x${original.height}');
